...
module Sidekiq
  module Debouncer
    class Enq < ::Sidekiq::Scheduled::Enq
      ...
      LUA_XPOP = File.read(File.expand_path("../lua/xpop.lua", __FILE__))

      define_lua_command(:zpopbyscore_withscore, LUA_ZPOPBYSCORE_WITHSCORE)
      define_lua_command(:xpop, LUA_XPOP)

      def enqueue_jobs
        redis do |conn|
          while !@done && (job, score = zpopbyscore_withscore(conn, keys: [SET], argv: [Time.now.to_f.to_s]))
            job_args = xpop(conn, keys: [job], argv: [score])
            final_args = job_args.map { |elem| Sidekiq.load_json(elem[1][1]) }
